...
const TUNE = {
  ...
  strength: 0.13, // rest refraction/distortion amount (raised 0.1→0.13 → the idle orb reads more refracted/unclear)
  boatZoom: 0.9, // boat draw size within the lens (0.9 → the sailboat reads a little smaller inside the orb)
  ...
  restScale: 0.6, // orb size (× base) at rest / cursor in a far corner (smallest)
  peakScale: 1.1, // orb size (× base) when the cursor is right on it (largest)
  revealRest: 0.82, // interior brightness at rest (a hair darker → idle orb reads a little more 'unclear')
  revealPeak: 0.92, // interior brightness at closest approach — kept low (≈ the orb at scale 0.9 before) so the grown orb never reads fully 'clear'
  nearStrength: 0.086, // refraction strength at closest approach — kept high (≈ the orb at scale 0.9
  //                      before) so the grown orb STAYS blurry/refracted instead of clarifying
  hudAlpha: 1.0, // peak HUD strength at full hover — 1.0 = the most contrasted (pure b/w)
  hudRevealStart: 0.8, // orb scale (hoverScale) where the whole HUD BEGINS to fade in
  hudRevealFull: 1.0, // HUD reaches full clarity at this fraction of peakScale (1.0 × max = orb scale 1.0)
  hudSize: 0.12, // title (LA 2028 / OLYMPICS) type size, as a fraction of orb diameter — dialed back a little
  //               from the too-big 0.134; still slightly oversized, auto-centered on the orb (drawHudMask)
  hudTimerRatio: 0.62, // countdown size relative to the title (< 1 → title reads larger)
  ...
  boatMaxScale: 1.1, // hard cap on the boat's size (× its base) — matches peakScale
